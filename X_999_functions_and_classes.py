...
class MyDataset(Dataset):
    def __init__(self, X, targets):
        self.X = torch.tensor(X, dtype=torch.float32)# Convert the NumPy array to a PyTorch tensor
        self.targets = torch.tensor(targets, dtype=torch.float32)# Convert the NumPy array to a PyTorch tensor
    def __len__(self):
        return len(self.X)
